...
def graph_water_transformer_cov_process_for_gate_predictor(n_hours, K, masked_value, split_1, split_2):
    ...
    X_mask_reshape = X_mask.reshape((X_mask.shape[0], -1)) # (96337, 1536)
    # print("X_mask_reshape: ", X_mask_reshape.shape)
    ws_true_reshape = ws_true.reshape((ws_true.shape[0], -1)) # (96337, 96)
    # print("ws_true_reshape: ", ws_true_reshape.shape)
    gate_true_reshape = gate_true.reshape((gate_true.shape[0], -1)) # (96337, 168)
    # print("gate_true_reshape: ", gate_true_reshape.shape)
    ...
    # final train / val / test
    train_X_mask = train_X_mask_scaled.reshape((-1, n_hours+K, features)) # (77069, 96, 16)
    # print("train_X_mask", train_X_mask.shape)
    val_X_mask = val_X_mask_scaled.reshape((-1, n_hours+K, features))
    test_X_mask = test_X_mask_scaled.reshape((-1, n_hours+K, features))

    train_ws_y = train_ws_true_scaled # (77069, 96)
    # print("train_ws_y", train_ws_y.shape)
    val_ws_y = val_ws_true_scaled
    test_ws_y = test_ws_true_scaled
    
    train_gate_pump_y = train_gate_true_scaled # (77069, 168)
    # print("train_gate_pump_y", train_gate_pump_y.shape)
    val_gate_pump_y = val_gate_true_scaled
    test_gate_pump_y = test_gate_true_scaled
    
    train_gate_pump_y = train_gate_pump_y.reshape((-1, 24, 7)) # (77069, 24, 7)
    # print("train_gate_pump_y", train_gate_pump_y.shape)
